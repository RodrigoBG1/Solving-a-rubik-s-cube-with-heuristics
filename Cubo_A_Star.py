...
class Heuristics:
    ...
    @staticmethod
    #Esta es para saber cuántas esquinas tienen por al menos una cara mal
...
class World:
    ...
    def solve_a_star(self, source_cube, target_cube, heuristic):
        ...
        while not pq.empty():
            ...
            #Checo si la tupla de un nodo es igual a la del target, es decir, veo si el cubo ya esta armado
            if curr_node.cube_int_tuple == target.cube_int_tuple:
                self.print_solution(curr_node)
                return

            #Este 'if' es para que si un estado del cubo ya se visitó ya no se saquen los estados del cubo iguales que existen en la pq
            if self.is_visited(curr_node.cube_int_tuple):
                continue
            else:
                self.visited.add(curr_node.cube_int_tuple)

                for i in range(len(self.movement_12)):
                    #En la clase Cube_Movements necesita recibir un cubo y regresarlo ya modificado
                    #Necesita tener 3 funciones, la de suffle, la de revolver manualmente y la de hacer un movimiento(esta ultima es para el bfs)
                    ...
                    #Este 'if' es para ya no seguir creando más cubos si ya han sido vistidos, es decir, para no crear cubos visitados repetidos
        ...
        for k in range(6):
            for i in range(2,-1,-1):
                for j in range(2,-1,-1):
                    x = x << 3 | cube[k][i][j] #Shift de 3 a la izquierda
            list[k] = x
            x = 0
        return tuple(list) #Esa lista de enteros la convierto a una tupla
